...
form.addEventListener('submit', function(event) {
    ...
    // Cache-busting query string versioning
    const cacheBustVersion = '1.1'; // You can increment this value when needed
    ...
    if (name === 'sevde' || name === 'sevdenur' || name === 'sevde nur') {
        ...
        // Content for Sevde
        outputTitle.textContent = `${name.charAt(0).toUpperCase() + name.slice(1)}, sen ayransın!`;
        outputImage.src = `https://www.nazar.com.tr/wp-content/uploads/2020/12/ayran.jpg?v=${cacheBustVersion}`; // Cache-busted image URL
        outputImage.style.display = 'block';
        outputText.textContent = "Ayran... Bu toprakların milli içkisi. En güzel sofraların eşlikçisi, 7'den 70'e herkesin gönlünü çelen biricik tercih. Sütten yoğurda dönüşen ardından son haline ulaşan hayat yolculuğuyla bu içecek emeğin, evrimin ve daima daha iyiye gidişin Türk mutfağındaki sembolü. Muhafazakar bir kızın olabileceği en iyi içecek!";
            
        // Play Sevde's specific music with cache-busting
        musicSource.src = `audio/sevde-music.mp3?v=${cacheBustVersion}`; 
        backgroundMusic.style.display = 'block';
        backgroundMusic.load();
        backgroundMusic.play();
        ...
        // Content for lavuk
        outputTitle.textContent = `${name.charAt(0).toUpperCase() + name.slice(1)}, sen deve sidiğisin!`;
        outputImage.src = `img/deve.png?v=${cacheBustVersion}`; // Cache-busted image URL
        outputImage.style.display = 'block';
        outputText.textContent = "Söylenecek fazla söz yok, çünkü hepsi kelime israfı. Sen bir loser'sın. Şansını kullanamayan ve sonradan çok üzülecek (olması gerektiği gibi) bir loser. Gelecek, senin için parlak değil. Kendini toplumdan tecrit et ve aynasız bir mahzende kimseye zarar vermeden ömrünü tamamla. Ne demişler, deve şaraptan ne anlar?";
        
        // Play specific music for lavuk with cache-busting
        musicSource.src = `audio/lavuk-music.mp3?v=${cacheBustVersion}`; 
        backgroundMusic.style.display = 'block';
        backgroundMusic.load();
        backgroundMusic.play();
        ...
        // Default content for any other name
        outputTitle.textContent = `${name.charAt(0).toUpperCase() + name.slice(1)}, sen Beypazarı Maden Suyu'sun!`;
        outputImage.src = `img/beypazari.png?v=${cacheBustVersion}`; // Cache-busted image URL
        outputImage.style.display = 'block';
        outputText.textContent = "Siteyi yetiştirmem lazım, çok düşünemedim. Ama bu site eline geçtiyse sen de fena biri değilsindir!";
